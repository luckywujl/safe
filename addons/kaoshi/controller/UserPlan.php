...
class UserPlan extends Controller
{
    ...
    {
        //        查找plan表中全部学生答题计划
        $public_plan = Db::name('KaoshiPlan')->where(['limit'=>0,'type'=>1])->column('id');
        if(is_array($public_plan) && count($public_plan)>0){
            $public_plan_ids = implode(',',$public_plan);
            $user_plan = $this->model->where(['plan_id'=>['in',$public_plan_ids],'user_id'=>$this->auth->id])->column('plan_id');
            $insert_msg = [];
            foreach ($public_plan as $v){
                if(!in_array($v,$user_plan))array_push($insert_msg,['user_id'=>$this->auth->id,'plan_id'=>$v,'status'=>0]);
            }
            $result = Db::name('KaoshiUserPlan')->insertAll($insert_msg);
    ...
    {
//        查找plan表中全部学生答题计划
        $public_plan = Db::name('KaoshiPlan')->where(['limit'=>0,'type'=>0])->column('id');
        if(is_array($public_plan) && count($public_plan)>0){
            $public_plan_ids = implode(',',$public_plan);
            $user_plan = $this->model->where(['plan_id'=>['in',$public_plan_ids],'user_id'=>$this->auth->id])->column('plan_id');
            $insert_msg = [];
            foreach ($public_plan as $v){
                if(!in_array($v,$user_plan))array_push($insert_msg,['user_id'=>$this->auth->id,'plan_id'=>$v,'status'=>0,'company_id'=>$this->auth->company_id]);
            }
            $result = Db::name('KaoshiUserPlan')->insertAll($insert_msg);
