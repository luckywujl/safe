...
class Category extends Backend
{
    ...
    /**
     * Category模型对象
     * @var \app\admin\model\materials\Category
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\materials\Category;
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $pcategory = $this->model->field('name')->where(['id'=>$params['pid'],'company_id'=>$this->auth->company_id])->find();
                $params['pname'] = '无';
                $params['pid']=0;
                $result = false;
                Db::startTrans();
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            $result = 0 ;
            //验证是否有分类资料未删除
            $infomodel = new \app\admin\model\materials\Info;
            $result = $infomodel->where(['materials_category_id'=>['in',$ids]])->select();
            if($result){
                $this->error(__('删除失败，原因是要删除的分类下有资料，请先删除它们'));
            }
            $pk = $this->model->getPk();
            $adminIds = $this->getDataLimitAdminIds();
            if (is_array($adminIds)) {
