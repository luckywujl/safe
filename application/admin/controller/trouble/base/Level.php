...
class Level extends Backend
{
    ...
    /**
     * Type模型对象
     * @var \app\admin\model\trouble\base\Level
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\trouble\base\Level;
    ...
    public function del($ids = "")
    {
        ...
        if ($ids) {
            ...
            $result = $mainmodel->where(['level'=>['in',$ids]])->select();
            if($result){
                $this->error(__('删除失败，原因是要删除的隐患等级正在被使用，不能删除！'));
            }
            $pk = $this->model->getPk();
            $adminIds = $this->getDataLimitAdminIds();
            if (is_array($adminIds)) {
